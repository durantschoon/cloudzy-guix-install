...
func (s *Step02MountExisting) RunWarnings(state *State) error {
	...
	fmt.Println()
	fmt.Println("ISO Cleanup: Ensures correct filesystem structure after copying from ISO.")
	fmt.Println("             Prevents service failures and boot issues on fresh install.")
	fmt.Println()
