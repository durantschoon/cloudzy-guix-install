...
EOF
            ...
            warn "   The GDM login screen uses the default keyboard layout (no options)."
            warn "   If you can't log in:"
            warn "   1. Switch to text console (Ctrl+Alt+F3)"
            warn "   2. Log in there (uses correct layout from config.scm)"
            warn "   3. Reset password: passwd yourusername"
            warn "      (Type password as if Caps Lock/Ctrl are NOT swapped)"
            warn "   4. Switch back to graphical login (Alt+F7)"
            warn "   Note: You don't need to change config.scm - just reset the password"
