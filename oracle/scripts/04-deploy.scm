  ...
  (let ((image-path
    ...
    (let* ((image-ocid (ensure-imported-image))
      ...
      (let ((ip (public-ip-of instance-ocid)))
        ...
        (say "")
        (say "First task on the instance: run lsblk and check whether the")
        (say "boot volume is /dev/sda or /dev/vda, and fix (targets ...) in")
        (say "oracle/image/oracle-image.scm to match BEFORE the first")
        (say "guix system reconfigure.  See oracle/README.md.")))))
